...
class Reports:
    ...
    m=(['Winter', 'Spring', 'Summer', 'Autumn'][(int(str(scd).split('-')[1])//3)]),
                                    ...
                                    'Relevance of Threat': Utils.risk_val(req['severity'], 'POAM'),
                        ...
                        for req in filter(lambda x: x['pluginId'] == plugin, host['requirements']):
                            if not list(filter(lambda x: str(x['Vulnerability ID(16a.3)']).strip() == f"{req['pluginId']}", report)):
                                ...
                                req_data = {
									...
									'Relevance of Threat (VL-VH) (16e)': Utils.risk_val(req['severity'], 'VL-VH'),
...
{comments}""".format(
    ...
    def rpt_raw_data(self):
        ...
        worksheet.set_column('Q:Q', 75)
        worksheet.set_column('R:R', 15)
        worksheet.set_column('S:S', 15)
        worksheet.set_column('T:T', 75)
        worksheet.set_column('U:U', 75)

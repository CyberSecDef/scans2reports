...
class Scans2Reports:
    ...
    def __init__(self, args):
        ...
        self.scar_data.set('skip_info', args.skip_info)
        self.scar_data.set('scd', args.scd)
        self.scar_data.set('lower_risk', args.lower_risk)
        self.scar_data.set('exclude_plugins', args.exclude_plugins)
        self.scar_data.set('test_results', args.test_results)
        self.scar_data.set('include_finding_details', args.finding_details)
        self.scar_data.set('predisposing_conditions', (args.predisposing_conditions if 'predisposing_conditions' in args and args.predisposing_conditions is not None and str(args.predisposing_conditions).strip() != '' else '') )
    ...
    def clean_up(self):
        for pickle in list( Path( self.scar_conf.get('application_path') ).glob('data/*.pkl') ):
            os.unlink( pickle )
...
required.add_argument('-x', '--exclude-plugins', help='Exclude plugins newer than this number of days', type=int, default=30)
required.add_argument('-t', '--threads', help='How intensive should the generator run (1-3)', type=int, default=2)
